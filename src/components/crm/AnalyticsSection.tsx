...
function AreaChart({ data, color, height = 120, isDark }: { data: { label: string; value: number }[]; color: string; height?: number; isDark: boolean }) {
  ...
  if (data.length < 2) return (
    <div style={{ height, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <p style={{ fontSize: '12px', color: isDark ? '#667085' : '#98a2b3' }}>Sem dados suficientes</p>
    </div>
  )
      ...
      <svg width="100%" height={height} viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ display: 'block' }}>
        ...
        {[0.25, 0.5, 0.75].map((f) => (
          <line key={f} x1="0" y1={H - f * (H - 10)} x2={W} y2={H - f * (H - 10)}
            stroke={isDark ? 'rgba(255,255,255,0.05)' : '#f3f4f6'} strokeWidth="0.5" />
        ))}
        ...
        {data.map(({ label, value }, i) => {
          ...
          return (
            <div key={label} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }} title={value > 0 ? fmtFull(value) : '—'}>
              <div style={{ width: '100%', height: `${h}px`, backgroundColor: isCurrent ? '#2c5545' : color, borderRadius: '3px 3px 0 0', transition: `height 0.5s cubic-bezier(0.16,1,0.3,1) ${i * 40}ms`, opacity: isCurrent ? 1 : 0.7 }} />
            </div>
          )
        })}
      </div>
...
function StatCard({ label, value, sub, icon: Icon, accent, trend, isDark }: {
  ...
  const mt = '#667085'
  const trendColor = !trend ? mt : trend.dir === 'up' ? '#2c5545' : trend.dir === 'down' ? '#c94444' : mt
  ...
  return (
    <div style={{ backgroundColor: bg, border: `1px solid ${bd}`, borderRadius: '12px', padding: '20px', boxShadow: isDark ? 'none' : '0 1px 3px rgba(16,24,40,0.06)' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '16px' }}>
        ...
        {trend && (
          <span style={{ fontSize: '11px', fontWeight: 600, color: trendColor, backgroundColor: trendBg, borderRadius: '6px', padding: '3px 8px', display: 'flex', alignItems: 'center', gap: '3px' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <div style={{ width: '3px', height: '18px', backgroundColor: isDark ? '#242422' : '#e4e0da', borderRadius: '99px' }} />
          <div>
            <p style={{ fontSize: '14px', fontWeight: 700, color: text, letterSpacing: '-0.02em' }}>Análise de Performance</p>
            <p style={{ fontSize: '11px', color: muted, marginTop: '1px' }}>Resultados e métricas do período seleccionado</p>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: '12px' }}>
        <StatCard label="Ganhos no Período" value={fmtFull(wonTotal)} sub={`${closedWon.length} deals fechados`} icon={DollarSign} accent="#2c5545" trend={{ dir: wonTotal > 0 ? 'up' : 'flat', pct: Math.round(goalPct) }} isDark={isDark} />
        <StatCard label="Meta Atingida" value={`${goalPct.toFixed(1)}%`} sub={`Meta: ${fmt(quarterlyGoal)}`} icon={Target} accent={goalPct >= 100 ? '#2c5545' : goalPct >= 60 ? '#a88030' : '#6b1212'} trend={{ dir: goalPct >= 100 ? 'up' : goalPct >= 60 ? 'flat' : 'down', pct: Math.round(goalPct) }} isDark={isDark} />
        <StatCard label="Win Rate" value={`${winRate}%`} sub={`${closedWon.length} ganhos · ${closedLost.length} perdidos`} icon={Award} accent={winRate >= 40 ? '#2c5545' : '#6b1212'} trend={{ dir: winRate >= 40 ? 'up' : 'down', pct: winRate }} isDark={isDark} />
                  ...
                  <div key={o.name} style={{ padding: '10px 20px', borderBottom: rank < ownerRanking.length - 1 ? `1px solid ${border}` : 'none' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 52px 64px 76px', gap: '8px', alignItems: 'center', marginBottom: '6px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
                        <span style={{ fontSize: '11px', fontWeight: 700, color: rank === 0 ? '#a88030' : muted, minWidth: '14px' }}>#{rank + 1}</span>
                      ...
                      <span style={{ fontSize: '12px', fontWeight: 600, color: '#2c5545', textAlign: 'right' }}>{o.wonCount}</span>
          ...
          {STAGES.filter((s) => !s.is_closed).map((stage) => {
            ...
            return (
              <div key={stage.id} style={{ backgroundColor: track, borderRadius: '10px', padding: '14px', border: `1px solid ${border}` }}>
                ...
                <p style={{ fontSize: '22px', fontWeight: 700, color: isHot ? '#6b1212' : text, fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.02em' }}>{avg !== null ? `${avg}d` : '—'}</p>
                <p style={{ fontSize: '10px', color: muted, marginTop: '2px' }}>{sd.length} deal{sd.length !== 1 ? 's' : ''}</p>
                {isHot && <p style={{ fontSize: '9px', fontWeight: 600, color: '#6b1212', marginTop: '4px' }}>⚠ Atenção</p>}
              </div>
            )
          })}
        </div>
